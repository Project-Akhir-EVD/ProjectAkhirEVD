...
createRegressionPlot <- function(data, var_x, var_y, lab_x, lab_y, show_reg_line, show_smooth_line, smoothness = 0.5, judul=NULL) {
  ...
  if (show_smooth_line) {
    p <- p + geom_smooth(method = "loess", se = FALSE, color = "red", span = smoothness)
  }
  return(p)
}
...
server <- function(input, output){
  ...
  output$scatter <- renderPlotly({
    if (!is.null(input$iv) && !is.null(input$dv) && is.numeric(myData()[, input$iv]) && is.numeric(myData()[, input$dv])) {
      ...
      ggplotly(createRegressionPlot(data = dataa, var_x = "x", var_y = "y", lab_x = input$iv, lab_y = input$dv, show_reg_line = input$linear, show_smooth_line = input$smooth, judul=titel))
    }
  })
}
